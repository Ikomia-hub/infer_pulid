...
from infer_pulid.PuLID.eva_clip import create_model_and_transforms
from infer_pulid.PuLID.eva_clip.constants import OPENAI_DATASET_MEAN, OPENAI_DATASET_STD
from infer_pulid.PuLID.pulid.encoders import IDEncoder
from infer_pulid.PuLID.pulid.utils import is_torch2_available

if is_torch2_available():
    from infer_pulid.PuLID.pulid.attention_processor import AttnProcessor2_0 as AttnProcessor
    from infer_pulid.PuLID.pulid.attention_processor import IDAttnProcessor2_0 as IDAttnProcessor
else:
    from infer_pulid.PuLID.pulid.attention_processor import AttnProcessor, IDAttnProcessor
...
class PuLIDPipeline:
    def __init__(self, *args, **kwargs):
        ...
        self.weight_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.realpath(__file__)))), "models")

        # load base model
        unet = UNet2DConditionModel.from_config(sdxl_base_repo, subfolder='unet').to(self.device, torch.float16)
        unet.load_state_dict(
            load_file(
                hf_hub_download(sdxl_lightning_repo, 'sdxl_lightning_4step_unet.safetensors', local_dir=self.weight_dir), device=self.device
            )
        )
        self.hack_unet_attn_layers(unet)
        self.pipe = StableDiffusionXLPipeline.from_pretrained(
            sdxl_base_repo, unet=unet, torch_dtype=torch.float16, variant="fp16",
            cache_dir = self.weight_dir
        ).to(self.device)
        ...
        # clip-vit backbone
        model, _, _ = create_model_and_transforms('EVA02-CLIP-L-14-336', 'eva_clip', force_custom_clip=True, cache_dir=self.weight_dir)
        model = model.visual
    ...
    def load_pretrain(self):
        ...
        hf_hub_download('guozinan/PuLID', 'pulid_v1.bin', local_dir=self.weight_dir)
        ckpt_path = os.path.join(self.weight_dir, 'pulid_v1.bin')
